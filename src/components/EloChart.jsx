  ...
  return (
    <div className="rounded-xl border border-gray-700 bg-gray-900 px-3 py-2 shadow-lg text-sm min-w-[120px]">
      ...
      <div className="flex items-baseline gap-1.5">
        <span className={`text-base font-bold ${d.diff > 0 ? 'text-emerald-400' : d.diff < 0 ? 'text-red-400' : 'text-gray-400'}`}>
          {d.diff > 0 ? '+' : ''}{d.diff}
        </span>
...
export function EloChart({ getChartData, loading, onExportPGN, blitzPerf, className = '' }) {
  ...
  const maxAbs    = data.length ? Math.max(...data.map((d) => Math.abs(d.diff)), 1) : 10
  ...
  return (
    <Card className={`flex flex-col ${className}`}>
      <CardHeader className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {totalGames > 0 && (
            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
              ...
              <span className={`font-semibold text-sm ${totalDiff > 0 ? 'text-emerald-400' : totalDiff < 0 ? 'text-red-400' : 'text-gray-500'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {onExportPGN && totalGames > 0 && (
            ...
            >
              {copying ? <span className="animate-pulse">...</span>
      ...
      <CardContent className="flex-1 flex gap-3 min-h-0">
        <div className="flex-1 min-h-0 min-w-0">
          {loading ? (
            <div className="flex items-center justify-center h-full min-h-[180px] text-gray-600 text-sm animate-pulse">
              Cargando partidas...
            </div>
          ) : totalGames === 0 ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[180px] text-gray-600 text-sm gap-2">
              <span className="text-3xl">♟</span>
              <span>
                {view === 'day' ? 'No jugaste partidas hoy'
                  : view === 'week' ? 'No jugaste partidas esta semana'
              ...
              <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} barCategoryGap="20%">
                ...
                <Bar dataKey="diff" radius={[3, 3, 3, 3]}>
                  {data.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.diff > 0 ? '#34d399' : entry.diff < 0 ? '#f87171' : '#6b7280'}
                      fillOpacity={0.85}
                    />
                  ))}
                </Bar>
